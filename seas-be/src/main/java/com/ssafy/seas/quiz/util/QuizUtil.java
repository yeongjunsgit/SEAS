...
public class QuizUtil {
    ...
    public double[][] getPrefixWeightMap(List<QuizDto.QuizWeightInfo> weightInfo){
        ...
        quizWeight[0][1] = weightInfo.get(0).getQuizInterval() * weightInfo.get(0).getEf();
    ...
    public double[] selectQuizzes(double[][] quizWeightInfo) {
        ...
        double random = toFixed(Math.random() * (max - min) + min, 1); // 1.3 ~ max 구간의 랜덤값이 나옴

        int selectedQuizIndex = Arrays.binarySearch(weightArray, random);

        // binarySearch에서는 정확한 값이 아니면, 음수로 이 값이 있을 시 위치할 인덱스를 돌려주므로 굿
        ...
        selectedQuizInfo[1] = quizWeightInfo[selectedQuizIndex][1]; // 생각해보니 굳이 weight를 담을 필요가?
